...
def pack_pathway_output(cfg, frames, audio_frames=None, flow_frames=None, text_tokens=None):
    """
    ...
    def sample_slow_pathway(frames):
    ...
    if cfg.MODEL.ARCH in cfg.MODEL.SINGLE_PATHWAY_ARCH:
        if cfg.MODEL.ARCH in {'audionet', 'a2d'}:
            frame_list = [audio_frames]
        else:
            frame_list = [frames]
    elif cfg.MODEL.ARCH in cfg.MODEL.MULTI_PATHWAY_ARCH:
        if cfg.MODEL.ARCH == "slowfast":
            slow_pathway = sample_slow_pathway(frames)
            frame_list = [slow_pathway, frames]
        elif cfg.MODEL.ARCH == "avslowfast":
            slow_pathway = sample_slow_pathway(frames)
            frame_list = [slow_pathway, frames, audio_frames]
        elif cfg.MODEL.ARCH == "avnet":
            frame_list = [frames, audio_frames]
        elif cfg.MODEL.ARCH == "avnet_small_conv1":
            frame_list = [frames, audio_frames]
        elif cfg.MODEL.ARCH == "avnet_c2d":
            frame_list = [frames, audio_frames]
        elif cfg.MODEL.ARCH == "video_flow":
            frame_list = [frames, flow_frames]
        elif cfg.MODEL.ARCH == "video_flow_3d":
            frame_list = [frames, flow_frames]
        elif cfg.MODEL.ARCH == "video_flow_T11133":
            frame_list = [frames, flow_frames]
        elif cfg.MODEL.ARCH == "video_flow_transformer_cnn":
            frame_list = [frames, flow_frames]
        elif cfg.MODEL.ARCH == "c2d_text":
            frame_list = [frames, text_tokens]
    else:
...
def spatial_sampling(
    ...
    Args:
        ...
        spatial_idx (int): if -1, perform random spatial sampling. If 0, 1,
            ...
            than width. If -2, perform jitter on both height and width.
    ...
    """
    assert spatial_idx in {-3, -2, -1, 0, 1, 2}
    if spatial_idx in {-1, -2, -3}:
        if spatial_idx == -1:
            ...
            frames = transform.random_scale_jitter(
                images=frames,
                min_size=min_scale,
                max_size=max_scale,
                inverse_uniform_sampling=inverse_uniform_sampling,
            ...
            frames = transform.random_area_ar_jitter(
                images=frames,
            ...
            raise NotImplementedError("Does not support spatial_idx={}.".format(spatial_idx))
        if random_horizontal_flip:
            frames, _ = transform.horizontal_flip(0.5, frames)
        if color_jitter is not None and color_jitter['prob'] > 0.0 and random.random() < color_jitter['prob']:
            frames = transform.color_jitter(
                frames, 
                img_brightness=color_jitter['brightness'], 
                img_contrast=color_jitter['contrast'], 
                img_saturation=color_jitter['saturation'],
            )
        if grayscale is not None and grayscale['prob'] > 0.0 and random.random() < grayscale['prob']:
            frames = transform.grayscale(frames)
        if gaussian_blur is not None and gaussian_blur['prob'] > 0.0 and random.random() < gaussian_blur['prob']:
            if 'motion_blur_prob' in gaussian_blur and random.random() < gaussian_blur['motion_blur_prob']: 
                frames = transform.motion_blur(frames)
            else:
                frames = transform.gaussian_blur(frames, gaussian_blur['sigma'])
    else:
...
def tensor_normalize(tensor, mean, std):
    ...
    """
    tensor = image_int_to_float(tensor)
    if type(mean) == list:
