...
def distributed_scale_lr(cfg):
    if cfg.NUM_SHARDS > 1:
        logger = logging.getLogger(__name__)
        logger.info(
            "cfg.NUM_SHARDS={}, Scale LR {} --> {}".format(
                cfg.NUM_SHARDS, cfg.SOLVER.BASE_LR, cfg.SOLVER.BASE_LR * cfg.NUM_SHARDS
            )
        )
        logger.info(
            "cfg.NUM_SHARDS={}, Scale WARMUP_START_LR {} --> {}".format(
                cfg.NUM_SHARDS, cfg.SOLVER.WARMUP_START_LR, cfg.SOLVER.WARMUP_START_LR * cfg.NUM_SHARDS
            )
        )
